...
@section('body')
    <div class="bg-white rounded-lg border border-gray-200">
        <h2 class="font-semibold text-xl p-6">Detail Metode Pembayaran</h2>
        <dl class="border-b border-gray-100">
            ...
            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                    {{ $metode_pembayaran->kode ?? '-' }}
                </dd>
            </div>
        </dl>

        <div class="flex items-center space-x-1 justify-start p-6">
            <a href="{{ route('metode-pembayaran.edit', ['metode_pembayaran' => $metode_pembayaran->id]) }}"
            ...
            <form method="POST" action="{{ route('metode-pembayaran.destroy', ['metode_pembayaran' => $metode_pembayaran->id]) }}">
                @method('DELETE')
                @csrf

                <a :href="{{ route('metode-pembayaran.destroy', ['metode_pembayaran' => $metode_pembayaran->id]) }}"
