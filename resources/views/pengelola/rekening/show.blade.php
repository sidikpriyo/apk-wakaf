...
@section('body')
    <div class="bg-white rounded-lg border border-gray-200">
        <h2 class="font-semibold text-xl p-6">Detail Rekening</h2>
        <dl class="border-b border-gray-100">
            ...
            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                    {{ $rekening->bank()->first()->nama ?? '-' }}
                </dd>
            </div>
        </dl>

        <div class="flex items-center space-x-1 justify-start p-6">
            <a href="{{ route('rekening.edit', ['rekening' => $rekening->id]) }}"
            ...
            <form method="POST" action="{{ route('rekening.destroy', ['rekening' => $rekening->id]) }}">
                @method('DELETE')
                @csrf

                <a :href="{{ route('rekening.destroy', ['rekening' => $rekening->id]) }}"
