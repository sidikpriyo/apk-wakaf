...
@section('body')
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        <h2 class="font-semibold text-xl mb-6">Edit Lembaga</h2>
        <form action="{{ route('lembaga.update', ['lembaga' => $lembaga->id]) }}" method="POST">
            @method('PATCH')
            @csrf
            <div>
                ...
                <x-input id="name" class="block mt-1 w-full border border-gray-200 p-1" type="text" name="name"
                    :value="$lembaga->name" required autofocus />
            </div>
