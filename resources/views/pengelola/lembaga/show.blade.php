...
@section('body')
    <div class="bg-white rounded-lg shadow-lg">
        <h2 class="font-semibold text-xl p-6">Detail Lembaga</h2>
        ...
        <div class="flex items-center space-x-1 justify-start p-6">
            <a href="{{ route('lembaga.edit', ['lembaga' => $lembaga->id]) }}"
            ...
            <form method="POST" action="{{ route('lembaga.destroy', ['lembaga' => $lembaga->id]) }}">
                @method('DELETE')
                @csrf

                <a :href="{{ route('lembaga.destroy', ['lembaga' => $lembaga->id]) }}"
